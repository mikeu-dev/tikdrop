            ...
            <main className="container mx-auto px-4 py-8 md:py-12 min-h-screen">
                ...
                {isLoadingHistory ? (
                    ...
                    <div className="text-center py-20 bg-muted/20 rounded-xl border border-dashed">
                        <h2 className="text-xl font-semibold mb-2">{t('history.empty.title')}</h2>
                        <p className="text-muted-foreground mb-4">{t('history.empty.desc')}</p>
                        <Button onClick={() => router.push('/')}>{t('history.empty.button')}</Button>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {history.map((item, index) => (
                            ...
                            >
                                ...
                                <div className="p-4">
                                    <h3 className="font-semibold truncate mb-2" title={item.video.title}>{item.video.title || t('history.noTitle')}</h3>
                                    <div className="flex items-center justify-between text-sm text-muted-foreground mb-4">
                                        <div className="flex items-center gap-1">
                                            <Calendar className="h-3 w-3" />
                                            <span>{item.createdAt?.seconds ? format(new Date(item.createdAt.seconds * 1000), 'MMM d, yyyy') : t('history.justNow')}</span>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <Button size="sm" variant="outline" className="w-full flex gap-2" asChild>
                                            <a href={item.video.hdplay || item.video.play} target="_blank" rel="noopener noreferrer">
                                                <Download className="h-3 w-3" /> {t('history.download')}
                                            </a>
                                        </Button>
